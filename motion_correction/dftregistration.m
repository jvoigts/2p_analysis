...
function [output Greg Greg2 Greg3] = dftregistration(buf1ft,buf2ft,usfac,buf3ft,buf4ft)
% function [output Greg Greg2] = dftregistration(buf1ft,buf2ft,usfac);
...
% cad: I modified this in a trivial way to allow for registration of another channel for free (eg. red and green)
...
elseif (nargout > 1)&&(usfac > 0)&&(nargin==4),
    [nr,nc]=size(buf2ft);
    Nr = ifftshift([-fix(nr/2):ceil(nr/2)-1]);
    Nc = ifftshift([-fix(nc/2):ceil(nc/2)-1]);
    [Nc,Nr] = meshgrid(Nc,Nr);
    Greg = buf2ft.*exp(i*2*pi*(-row_shift*Nr/nr-col_shift*Nc/nc));
    Greg = Greg*exp(i*diffphase);
    Greg2 = buf3ft.*exp(i*2*pi*(-row_shift*Nr/nr-col_shift*Nc/nc));
    Greg2 = Greg2*exp(i*diffphase);
elseif (nargout > 1)&&(usfac > 0)&&(nargin==5),
    [nr,nc]=size(buf2ft);
    Nr = ifftshift([-fix(nr/2):ceil(nr/2)-1]);
    Nc = ifftshift([-fix(nc/2):ceil(nc/2)-1]);
    [Nc,Nr] = meshgrid(Nc,Nr);
    ...
    Greg2 = buf3ft.*exp(i*2*pi*(-row_shift*Nr/nr-col_shift*Nc/nc));
    Greg2 = Greg2*exp(i*diffphase);
    Greg3 = buf4ft.*exp(i*2*pi*(-row_shift*Nr/nr-col_shift*Nc/nc));
    Greg3 = Greg2*exp(i*diffphase);
